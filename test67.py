...
def find_shortest_path():
    ...
    try:
        ...
        # Save the GeoDataFrame to a MapInfo TAB file
        path_gdf.to_file(output_mapinfo_tab, driver="MapInfo File")
        ...
        print(f"Saved shortest path to {output_mapinfo_tab}")
...
# Function to select output MapInfo TAB file
def select_output_mapinfo_tab():
    global output_mapinfo_tab
    output_mapinfo_tab = filedialog.asksaveasfilename(defaultextension=".tab", filetypes=[("MapInfo TAB files", "*.tab")])
    if output_mapinfo_tab:
        messagebox.showinfo("Output MapInfo TAB", f"Selected output file: {output_mapinfo_tab}")
...
output_button = CTkButton(root, text="Save Output File", command=select_output_mapinfo_tab)
...
if india_shapefile_path and output_mapinfo_tab:
    ...
    print("Input or output file not selected. Exiting.")
